...
- Create a CronJob in `manifests/cronjob.yaml` named `postgres-backup` to automate daily backups of the PostgreSQL database. The job should:
  ...
  - Insert the command bellow to make the backup with timespamps
...
- To test the `postgres-backup` CronJob without waiting until midnight, create a one-time Job from the CronJob. This will allow you to trigger a backup immediately and verify that the setup is working correctly. Use a command to create a job from the existing `postgres-backup` CronJob, ensuring it runs with the same configuration.
